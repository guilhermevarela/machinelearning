...
if __name__ == '__main__':
    # this grid gives you a reward o f -0.1 for every non-terminal state
    ...
    while True:
        ...
        # policy evalution step - we already know how to do this!
        while True:
            biggest_change = 0 
            for s in states:
                old_v = V[s]

                # V(s) onlyh has value if it's not terminal state
                new_v = 0 
                if s in policy:
        ...
        for s in states:
            if s in policy:
                ...
                # loop throgh all possible actions to find the best current action
    ...
    print('policy:')
    print_policy(policy, grid)
    # result: every move is as bad as losingm so lose as quickly as possible
