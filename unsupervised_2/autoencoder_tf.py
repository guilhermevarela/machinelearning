...
motivation: Unsupervised techniques auto encoder + tensorflow
...
from util import get_mnist, error_rate

class AutoEncoder(object):
	def __init__(self, D, M, an_id): 
		self.M = M 
		self.id = an_id 
		# We have to define all variables in tensorflow before initializing them
		self.build(D, M) 
	...
	def build(self, D, M):
		#Define all TF variables and operations
...
class DnnTensorflow(object):			
	def __init__(self, D, hidden_layer_sizes, K, UnsupervisedModel=AutoEncoder):
		...
		self.build_final_layer(D, hidden_layer_sizes[-1], K) # logistic regression layer  
	...
	def fit(self, X, Y, Xtest, Ytest, pretrain=True, epochs=1, batch_sz=100):
		...
		#greesy layer-wise training of auto encoders
		pretrain_epochs=1
		if not pretrain:
			pretrain_epochs=0
		...
		for ae in self.hidden_layers:
			...
			#create current input from the next layer
